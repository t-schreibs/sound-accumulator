...
def try_scaffold_artist(artist, spotify):
    ...
    for release in releases:
        ...
        if not exists(release_name, current_releases):
            ...
            create_track_entries(filter(lambda track: not exists(track['name'], current_tracks), tracks), release['name']);
            result = True;
    return result;
...
def create_release_entry(release, default_genres):
    ...
    if not music_entries.try_generate_tracklist(release['name']):
        red(f'Could not generate tracklist for {release['name']}. Is this a duplicate entry?');
