        ...
        plugins.push(() => {
          return (tree: Root, _file) => {
            findAndReplaceMarkdown(tree, WikilinkRegex, (value: string, ...capture: string[]) => {
              ...
              else if (linkInfo.Type === "embed") {
                return {
                  type: "html",
                  data: { hProperties: { transclude: true } },
                  value: `<blockquote class="transclude" data-url= "${linkInfo.Url}" data-block="${linkInfo.Block}"><a href="${
                    linkInfo.Url + linkInfo.Anchor
                  }" class="transclude-inner">Transclude of block ${linkInfo.Block}</a></blockquote>`
                }
              }
              else {
